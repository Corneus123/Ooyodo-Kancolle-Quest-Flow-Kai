$(function () {
  ...
  //TODO quand on update l'état d'une quête, certqines quetes se mettent pas en pending
...
document.querySelector(".HD_main_tab_btn[value='FC']").addEventListener("click", showFlowchart);
  ...
  function initialisation() {
    ...
    $(document).on("bubble_displayed", function (e, bubble) {
      if (bubble === "MSG_welcome") {
        ...
        //   var questCookie = {progress:'{"p":{"S":"B02"},"c":{"S":"B01"}}', undeterminedQuests:[], bookmark:[], timeStamp:moment().format()};
        //questCookie.timeStamp = "2017-10-21T18:00:00+09:00";
        ...
        if (!questStateCalculated) {
          $(document).trigger("start_new_functions");
        }
  ...
  function changeQuestState(quest) {
    var questsCookie = JSON.parse(getCookie('user_quests'));
    var previousState = ALL_QUEST_STATE[quest];
    var questsModified = { locked: [], pending: [], completed: [] };
    var questPeriod = periodNumberEquivalence(ALL_QUESTS_LIST[quest].period);
    var nextState;
    var changeOptions;
    ...
    displayBubbleMessage(`Admiral, which state do you want to set quest ${quest}?
      ${previousState !== "pending" ? "<br><button type='button' class='MSG_change_quest_state_btn' value='pending'>Pending</button>" : ""}
      ${previousState !== "completed" ? "<br><button type='button' class='MSG_change_quest_state_btn' value='completed'>Completed</button>" : ""}`
      , "writing", "MSG_change_quest_state", false, true, true);

    $(".MSG_change_quest_state_btn").click(function () {
      nextState = $(this).val();
      if (previousState === "locked") {
        displayBubbleMessage(`Understood, all the quests required by this one will be set to "completed".
          Do you also want to apply this change to periodic quests?
          <br><button type='button' class='MSG_change_quest_state_btn' value='yes_completed'>Yes, change everything</button>
          <br><button type='button' class='MSG_change_quest_state_btn' value='no'>No, change the minimum</button>`
          , "writing", "MSG_change_quest_state", false, true, true);
      } else if (previousState === "completed" && nextState === "pending") {
        displayBubbleMessage(`Understood, all the quests unlocked by this one will be set to "locked".
          Do you also want to apply this change to one time quests?
          <br><button type='button' class='MSG_change_quest_state_btn' value='yes_locked'>Yes, change everything</button>
          <br><button type='button' class='MSG_change_quest_state_btn' value='no'>No, change the minimum</button>`
          , "writing", "MSG_change_quest_state", false, true, true);
      } else if (previousState === "pending" && nextState === "completed") {
        displayBubbleMessage(`Understood.
            Do you also want to apply this change?
            <br><button type='button' class='MSG_change_quest_state_btn' value='no'>Yes</button>`
          , "writing", "MSG_change_quest_state", false, true, true
        );
      }

      $(".MSG_change_quest_state_btn").click(function () {
        closeBubbleMessage($("#MSG_change_quest_state"));
        changeOptions = $(this).val();
        ALL_QUEST_STATE[quest] = nextState;
        updateQuestStateDisplay(quest);
        questsModified[nextState].push(quest);

        // set the next quests to "locked"
        if (previousState === "completed" && nextState === "pending") {
          lockNextQuest(quest);

          function lockNextQuest(qst) {
            ALL_QUESTS_LIST[qst].unlocks.forEach(nxtQst => {
              var nxtQstPeriod = periodNumberEquivalence(ALL_QUESTS_LIST[nxtQst].period);
              if (((changeOptions === "yes_locked" && nxtQstPeriod > questPeriod) || nxtQstPeriod <= questPeriod) && ALL_QUEST_STATE[nxtQst] !== "locked") {
                ALL_QUEST_STATE[nxtQst] = "locked";
                updateQuestStateDisplay(nxtQst);
                questsModified.locked.push(nxtQst);
              }
              lockNextQuest(nxtQst);
            });
          }
        }

        // set the previous quests to "completed"
        if (previousState === "locked") {
          completePreviousQuest(quest);

          function completePreviousQuest(qst) {
            ALL_QUESTS_LIST[qst].requires.forEach(prvQst => {

              var prvQstPeriod = periodNumberEquivalence(ALL_QUESTS_LIST[prvQst].period);
              if (((changeOptions === "yes_completed" && prvQstPeriod < questPeriod) || prvQstPeriod >= questPeriod) && ALL_QUEST_STATE[prvQst] !== "completed") {
                ALL_QUEST_STATE[prvQst] = "completed";
                updateQuestStateDisplay(prvQst);
                questsModified.completed.push(prvQst);
              }
              completePreviousQuest(prvQst);
            });
          }
        }

        // if the quest is set to completed, check la suite to put to pending quests with qll requirements completed
            ...
            if (ALL_QUEST_STATE[unlockedQuest] === "locked") {
              if (ALL_QUESTS_LIST[unlockedQuest].requires.every(function (requiredQuest) { return ALL_QUEST_STATE[requiredQuest] === 'completed'; })) {
                ...
                if ($.inArray(unlockedQuest, questsCookie.undeterminedQuests) === -1) {
                  ...
                  questsModified.pending.push(unlockedQuest);
                } else {
        ...
        var msgHTML = "Admiral, here are the changes:";
        if (questsModified.completed.length > 0) {
          msgHTML += `Set to <b>completed</b>:<br><span id="MSG_quest_completed_quests">${questsWithLinks(questsModified.completed)}</span><br>`;
        }
        if (questsModified.pending.length > 0) {
          msgHTML += `Set to  <b>pending</b>:<br><span id="MSG_quest_pending_quests">${questsWithLinks(questsModified.pending)}</span><br>`;
        }
        if (questsModified.locked.length > 0) {
          msgHTML += `Set to <b>locked</b>:<br><span id="MSG_quest_locked_quests">${questsWithLinks(questsModified.locked)}</span><br>`;
        }
        displayBubbleMessage(msgHTML,
          "welcome", "MSG_quest_modified", false, false, true);
        addQuestLinkClickEvent($("#MSG_quest_modified"));
  ...
  function askToCheckIfQuestIsPendingInGame(unlockedQuest, visibleQuests) {
    ...
    function setAsPending() {
      ...
      displayBubbleMessage(`Admiral, you have unlocked the following quests:<br>
            <span id="MSG_quest_pending_quests">${questsWithLinks([unlockedQuest])}</span>`,
        "welcome", "MSG_quest_modified", false, false, true, function () { $("#MSG_quest_pending_quests").html(`${$("#MSG_quest_pending_quests").html()}, ${questsWithLinks([unlockedQuest])}`); }
      );
      addQuestLinkClickEvent($("#MSG_quest_pending_quests"));
    };
    ...
    $(`.MSG_check_unknown_quest_${unlockedQuest}_btn`).click(function () {
      ...
      if ($(this).val() === "completed") {
        //if not found it means it was already comnpleted so set all quest after to completed and remove the quest from the undertermined in the cookie
        var questsCookie = JSON.parse(getCookie('user_quests'));
        addUnlockedQuests(unlockedQuest).forEach(qst => {
          ALL_QUEST_STATE[qst] = "completed";
          updateQuestStateDisplay(qst);
          displayBubbleMessage(`Admiral, you have completed the following quests:<br>
                <span id="MSG_quest_completed_quests">${questsWithLinks([qst])}</span>`,
            "welcome", "MSG_quest_modified", true, false, true, function () { $("#MSG_quest_completed_quests").html(`${$("#MSG_quest_completed_quests").html()}, ${questsWithLinks([qst])}`); }
          );
        });
        questsCookie.undeterminedQuests.splice(questsCookie.undeterminedQuests.indexOf(unlockedQuest), 1);
        updateQuestListDisplay(visibleQuests);
  ...
  function displayQuestListSelect(questList) {
    ...
    optionsList.sort(function (a, b) {
      //added for the anniversary quests
      if (/5A0[0-9]/i.test(a.text) && /5A0[0-9]/i.test(b.text)){
        if(a.text.charAt(3) > b.text.charAt(3)) return 1;
        if(a.text.charAt(3) < b.text.charAt(3)) return -1;
      }
      if (/5A0[0-9]/i.test(a.text) && !/5A0[0-9]/i.test(b.text)) return -1;
      if (!/5A0[0-9]/i.test(a.text) && /5A0[0-9]/i.test(b.text)) return 1
      //
  ...
  function loadQuestStateFromCookie(userQuestCookie) {
    ...
    ALL_QUEST_STATE_TMP = checkQuestUpdates(userQuestCookie, ALL_QUEST_STATE_TMP);
  ...
  function displayRemainingQuests() {
    ...
    if (!$("#FC_RM_use_periodic_quests").is(":checked")) {
      remainingQuestsList = remainingQuestsList.filter(function (quest) { return ALL_QUESTS_LIST[quest].period === "once" });
    }
  ...
  function calculateQuestState(pendingQuests) {
    ...
    // set the "Unknown" quest to completed
    ALL_QUEST_STATE_TMP.Unknown = "completed";
    ...
    function setRemainingQuestState(period) {
      ...
      getQuestsInState(ALL_QUEST_STATE_TMP, '???').filter(function (qst) { return ALL_QUESTS_LIST[qst].period === period }).forEach(quest => {
        if (ALL_QUESTS_LIST[quest].requires[0] === "Unknown") {
          // if the quest have unknown requirement, leave it as ??? and let its state be asked to the user.
          ALL_QUEST_STATE_TMP[quest] = "???";
        } else if (ALL_QUESTS_LIST[quest].requires.every(function (requiredQuest) { return ALL_QUEST_STATE_TMP[requiredQuest] === "completed"; })) {
    ...
    function implementQuestsStateUpdated() {
      ...
      userQuestCookie.bookmark = bookmark;

      setCookie('user_quests', JSON.stringify(userQuestCookie), 365);
      var blockingQuests = getBlockingPeriodicQuests(undeterminedQuests);
      if (blockingQuests.length > 0) {
        displayBubbleMessage(`Admiral, about those quests that you didn't know the state, you should complete those quests:<br>
          <span id="MSG_quest_completion_advice_quests">${questsWithLinks(blockingQuests)}</span><br>
          Update your progress once you are done.`,
          "explaining", "MSG_quest_completion_advice", true, false, true);
  ...
  // return the compressed string with the progress to be saved in the cookie
  ...
  function getBlockedQuests() {
    ...
    return getQuestsInState(ALL_QUEST_STATE, "locked")
      ...
      .filter(function (qst) {
        return ALL_QUESTS_LIST[qst].requires.every(function (q) {
  ...
  function createQuestBox(questCode) {
    ...
    return `<div class="QL_questBox ${quest.period}" id='QL_questBox_${questCode}' style="background-color:${color}; color:${tinycolor(color).isLight() ? "#000000" : "#ffffff"};">
      ...
      <button type="button" class="QL_questBox_set_as" id='QL_set_as_${questCode}'>Set as...</button>


      </div>
      <div class="cellDiv" style=" height:75px;  top:40px; left:0px; width:100%;"}">
      <input type="checkbox" class="bookmark_checkbox" value="${questCode}" id="QL_bookmark_checkbox_${questCode}">
      <label for="QL_bookmark_checkbox_${questCode}"></label>
  ...
  // given the cookie saved in the last session, check if there was any update of the quest data file and edit the progress to incluide the new data
  function checkQuestUpdates(questCookie, ALL_QUEST_STATE_TMP) {
    // select the updates that occured since last connection.
    var newQuests = [];
    var removedQuests = [];
    var inconsistencies = [];
    Object.keys(updates).filter(function (u) {
      return (moment(questCookie.timeStamp).diff(moment(updates[u].implementationDate)) < 0);
    }).forEach(u => {
      var update = updates[u];
      // take all the new quests save them
      newQuests = newQuests.concat(update.questsAdded);
      removedQuests = removedQuests.concat(update.questsRemoved);
      update.questsRemoved.forEach(quest => {
        if ($.inArray(quest, newQuests) !== -1) {
          newQuests.splice(newQuests.indexOf(quest), 1);
        }
      });

      //check if there is any errors with the modified requirements of the quests
      //if there is any, ask for a recalculation of the progress
      Object.keys(update.questsReqModified).forEach(questMod => {
        if (update.questsReqModified[questMod].removed[0] === "Unknown" && ALL_QUEST_STATE_TMP[questMod] === "pending") {
          // if the modified quest was one that has unknown requirements and was in the default state pending (it's default when linked to the unknown quest)
          // in that case, it will be considered as a new quest and go through the same procedure
          newQuests.push(questMod);
        } else {
          //if not, check if the new requirements are OK for the state the quest is currently.
          if (ALL_QUEST_STATE_TMP[questMod] === "locked") {
            //if all its requirements are unlocked => error
            if (ALL_QUESTS_LIST[questMod].requires.every(function (req) { return ALL_QUEST_STATE_TMP[req] === "competed"; })) {
              inconsistencies.push(questMod);
            }
          } else {
            if (!checkCompletion(questMod)) {
              inconsistencies.push(questMod);
            }

            // if a quest of same or superior duration in its requirement tree is not completed
            function checkCompletion(quest) {
              var isOK = true;

              if (ALL_QUESTS_LIST[quest].requires.length !== 0) {
                ALL_QUESTS_LIST[quest].requires.forEach(req => {
                  if (periodNumberEquivalence(ALL_QUESTS_LIST[req].period) >= periodNumberEquivalence(ALL_QUESTS_LIST[quest].period)) {
                    // if same period, problem if not completed
                    if (ALL_QUEST_STATE_TMP[req] !== "completed") {
                      isOK = false;
                    }
                  } else {
                    // if smaller period and not completed, check the previous requirements
                    if (ALL_QUEST_STATE_TMP[req] !== "completed") {
                      isOK = isOK && checkCompletion(req);
                    }
                  }
                });
              }
    ...
    if (inconsistencies.length > 0) {
      // if problem present with the update => reset cookie + progress and ask for a new calculation of progress
      displayBubbleMessage(`Admiral, I'm so sorry. There was an update of the quest tree and it seems that
        your currently saved progress has some conflict with the updated quest tree.
        I have reinitialised your progress and must ask you to calculate again your progress to use this application without problems.
        <br><br>
        To make it easier for you, here is the list of the quests that were pending, if you want to copy them:
        <br>${getQuestsInState(ALL_QUEST_STATE_TMP, "pending").join(", ")}`
        , "shamed", "MSG_update_error", false, true, false);
      ...
      // add new quests to the progress and set their state depending to their requirements
      newQuests.forEach(quest => {
        if (ALL_QUESTS_LIST[quest].requires.every(function (req) { return ALL_QUEST_STATE_TMP[req] === "competed"; })) {
          ALL_QUEST_STATE_TMP[quest] = "pending";
        } else {
          ALL_QUEST_STATE_TMP[quest] = "locked";
        }
      ...
      removedQuests.forEach(quest => {
        delete ALL_QUEST_STATE_TMP[quest];
      });
  ...
  function activateQuestBoxesEventListenners() {
    // set the quest as completed
    $(".set_as, .QL_questBox_set_as").click(function () {
  ...
  function getQuestColor(quest) {
    ...
    // added to color right the anniversary quests
    if(/5A0[0-9]/i.test(quest)){
      switch(quest.charAt(3)){
        case '1': {
          questLetter = 'F';
          break;
        }
  ...
  function updateQuestListDisplay(questList) {
    ...
    $("#QL_displayed_state").text(displayedState.length === 3 ? "All" : displayedState.map(function (str) {
      return str.charAt(0).toUpperCase() + str.slice(1).replace(/ -/g, ':');
    }).join(", "));
    $("#QL_displayed_period").text(displayedPeriod.length === 5 ? "All" :
      displayedPeriod.length === 0 ? "None" : displayedPeriod.map(function (str) {
        return str.charAt(0).toUpperCase() + str.slice(1).replace(/ -/g, ':');
      }).join(", "));
  }
  ...
  function addQuestLinkClickEvent(JQueryElement) {
    JQueryElement.find(".questLink").click(function () {
      ...
      if ($("#FC").is(":visible")) {
        ...
        // get the list of currently displayed quests
        var displayedQuests = [];
        $("#FC_FT_select_quest_list option").each(function () {
          displayedQuests.push($(this).val());
        })
        //Add the clicked quest, its requirements and unlocks
        myDiagram.model.startTransaction("ShowClickedQuest");
        myDiagram.findNodeForKey(quest).visible = true;
        displayedQuests.push(quest);
        ALL_QUESTS_LIST[quest].unlocks.forEach(nextQuest => {
          myDiagram.findNodeForKey(nextQuest).visible = true;
          displayedQuests.push(nextQuest);
        });
        ALL_QUESTS_LIST[quest].requires.forEach(previousQuest => {
          myDiagram.findNodeForKey(previousQuest).visible = true;
          displayedQuests.push(previousQuest);
        });
        myDiagram.layoutDiagram(true);
        myDiagram.model.commitTransaction("ShowClickedQuest");
        //update the questListSelect
        displayQuestListSelect(removeDoublonFromArray(displayedQuests));
  ...
  function setQuestInputCase(input) {
    let inputCorrected = '';
    if (input.length >= 2) {
      if(/5A0[0-9]/i.test(input)){ // added to filter the anniversary quests 5A01
        inputCorrected = input.toUpperCase();
      } else if (/Su0[0-9]/i.test(input)) {
        inputCorrected = `${input.charAt(0).toUpperCase()}${input.charAt(1).toLowerCase()}${input.substring(2)}`;
      } else if (isNaN(parseInt(input.charAt(1), 10)) && input.charAt(0).toUpperCase() !== 'W' && input.charAt(0).toUpperCase() !== 'S') {
        inputCorrected = `${input.charAt(0).toUpperCase()}${input.charAt(1).toLowerCase()}${input.substring(2)}`;
      } else {
        inputCorrected = input.toUpperCase();
      }
  ...
  function questsWithLinks(questList) {
    var output = ""
    questList.sort(function (a, b) {
      if (a.replace(/[0-9]/g, '') > b.replace(/[0-9]/g, '')) return 1;
      if (a.replace(/[0-9]/g, '') < b.replace(/[0-9]/g, '')) return -1;
      if (a.replace(/[0-9]/g, '') == b.replace(/[0-9]/g, '')) {
        if (parseInt(a.replace(/^\D+/g, ''), 10) > parseInt(b.replace(/^\D+/g, ''), 10)) return 1;
        if (parseInt(a.replace(/^\D+/g, ''), 10) < parseInt(b.replace(/^\D+/g, ''), 10)) return -1;
      }
      return 0
    }).forEach(quest => {
  ...
  $("#QL_RM_select_search_method").change(function () {
    ...
    if ($(this).val() === "bookmark") {
      updateQuestListDisplay(bookmark);
    } else {
  ...
  $("#QL_RM_select_required_ship, #QL_RM_select_required_map, #QL_RM_search_select_reward, #QL_RM_select_required_equipment").change(function () {
  ...
  function resizeWindow() {
    var width = $(window).width() - 175 - 20; //remove right menu (175px) + margin (2*10px)
    var height = $(window).height() - 60 - 180 - 10 - 4;  //remove top header (60 px) + bottom menu if visible (180px) + boders (4px)
    if ($(window).width() <= 1400) { height -= 60; } //if the window is too small, the header become 60 px higher
    if ($("#FT_showHide").val() === "show") { height += 180; } //if the bottom menu is hidden, add 180 px to height
    resizeFlowchart(height, width);
  }

  // show or hide the footer
  $("#FT_showHide").click(function () {
    if ($(this).val() === "hide") {
      $(".FT").css("bottom", "-180px");
      $("#FT_showHide .arrow-up").show();
      $("#FT_showHide .arrow-down").hide();
      $(this).val("show");
    } else if ($(this).val() === "show") {
      $(".FT").css("bottom", "0px");
      $("#FT_showHide .arrow-up").hide();
      $("#FT_showHide .arrow-down").show();
      $(this).val("hide");
    }
    resizeWindow();
  });
  ...
  $(`#Ooyodo`).click(function () {
    displayBubbleMessage(`Yes Admiral, do you need something?<br>
          ...
          <button type="button" class="MSG_click_Ooyodo_btn" value="new_functions">What are the new functions implemented?</button>
      ...
      switch ($(this).val()) {
        ...
        case "reddit": {
          displayBubbleMessage(`Thanks a lot, feedbacks are always welcomed!<br>
                <a href="https://www.reddit.com/r/kancolle/comments/70m0jl/misc_ooyodos_quest_tracker/">Here is the link to the reddit thread.</a> <br>
